...
console = Console()
MAX_THREADS = 9999999988999999999999999999999
...
[bold blue]Box Zalo:[/] zalo.me/xxx""",
    ...
    # Lấy thời gian hiện tại (06:14 PM +07, Saturday, August 09, 2025)
    now = datetime.now()
    time_str = now.strftime("%I:%M %p %z")  # Định dạng 06:14 PM +0700
...
def send_otp_with_delay(func, phone, delay):
    try:
        func(phone)
        console.print(Panel(
            f"[green]✅ Spam thành công với {func.__name__}[/green]",
            title="THÀNH CÔNG", border_style="green"
        ))
    except Exception as e:
        console.print(Panel(
            f"[red]❌ Lỗi tại {func.__name__}: {e}[/red]",
...
delay = float(5)
repeat_count = int(input("Nhập số lần spam: "))  # Số lần gửi OTP
...
if __name__ == "__main__":
    ...
    for i in range(repeat_count):
        console.print(Panel(
            f"[cyan]📨 Vòng spam {i+1}[/cyan]",
            title="QUÁ TRÌNH", border_style="cyan"
        ))
        for func in otp_functions:
